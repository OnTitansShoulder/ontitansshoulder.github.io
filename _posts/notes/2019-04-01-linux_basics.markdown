...
Therefore, it is best to install Windows then Linux, since Windows will always take over the MBR, but Linux boot loader can be in MBR or other boot sector

**File System**
*Directory Tree* everything is from the root "/"
*mount* use a directory as mounting point for devices

**Different purpose of using Linux**
[more: book P.91]

**The Science in Partition**
analyze the computer's future purpose, then partition the disk for different Linux directories.
...
*Swap* is for swapping infrequently used things from Physical memory to Harddisk, to free some memory space for other applications.

**Linux X Window and Terminal Switching**
- [Ctrl]+[Alt]+[F1]~[F6] are pre-loaded tty1 ~ tty6 Terminal workspaces
...
*bc* can be a quick and light-weight calculator
- set `scale = 4` to make division precision (number of digits after decimal point)
- `quit` to leave

**End of File**
- [Ctrl]+[d] means End of File, End of Input. Can be used in place of entering `exit`
...
**Search for Man Page or Command**
...
**Info Page**
info page is a Linux specific feature that displays help doc like small paragraphs(pages), like a web-page.
There are lots of information about the page displayed, including the progress of viewing the entire doc.
like `man command`, use `info command`
...
**Use Nano**
like Vim, Nano is a very common and popular text editor for Linux.
...
- choose to shutdown, reboot, or enter single-user mode
...
*chown user_name:group_name file_name* can change owner and group access at the same time!
```
...
others = r-- = 4 + 0 + 0 = 4
`chmod 754 file_name`
*chmod* set access using symbols:
`chmod u/g/o/a +(add)/-(remove)/=(set) r/w/x file_name`
u=user, g=group, o=others, a=all!
`chmod u=rwx,go=rx .bashrc`

**interesting x**
'x' means execute, on a file means whether it can be executed, on a directory it means whether a user can `cd` into this directory as working directory
...
*unmask* means new files created will NOT have certain access rights
e.g. `unmask 023` means new files created will not have 'w' for groups and not have 'wx' for others
...
Hidden attributes are useful for security reasons.
...
For example, I have 'x' access to /usr/bin/passwd and passwd is owned by 'root'. When I execute passwd I temporarily get root access so I can change /etc/shadow
...
- when this user creates file under this directory, only this user or root can delete that file
...
**/usr directory**
*/usr* is short for Unix Software Resource, not 'user'!
Therefore, files in /usr should be shareable and static files.
This directory typical need a lot space.
Path|Content
----|-------
/usr/X11R6|X Window System files. The name implies X version being 11 and the sixth time released
...
/usr/sbin|non-system-critical commands, e.g. daemon for some network server
/usr/share|for sharing files
/usr/src|source code

**/var directory**
*/var* is used after the system is up, for the purpose of cache, log file, and other files generated by software including lock file and run file.
Path|Content
----|-------
...
- superblock: record this filesystem's information, including number of inode/block, used amount, remaining amount, filesystem format, etc.
- inode: record file-specific properties, for each record use one inode, and record the block number of this record
  - each 128 bytes
  - for large files, inode record block number has 12 direct block number, one redirect block number, and one triple-redirect block number. P247 Book.
- block: record the actual content of the file, may span to multiple blocks for larger files
- knowing the inode can know the block number.
- this way, data of multiple blocks can be read in sequence within a short amount of time
...
- large block may create many blocks not fully utilized

**inode/block bitmaps**
records and track used and unused blocks and inodes
helps fast lookup and find unused block/inode

- the process of reading a file at `/etc/passwd`:
  - filesystem find '/' inode
  - filesystem locate '/' block and look for etc inode
  - find 'etc' inode and check whether current user has 'rx' access
  - find 'etc' block and look for passwd inode
  - find 'passwd' inode and check whether current user has 'r' access
  - read passwd block

**Journaling filesystem**
during sudden power outage during writing to the disk, disk data and real data can be inconsistent. To deal with this and prevent a whole scan of the filesystem, a journaling filesystem helps by:
...
- hardlink to a file shares the original's inode
  - therefore, hardlink has the same access rights of the original
  - original inode exists as long as there is pointer to this inode
  - content not lost if original file is deleted
- softlink is just a name of another file.
  - can span to different filesystem
  - can work on directory
  - if original file deleted, can't work anymore

use *ln* to make hard/soft links
...
- It is best to partition in single-user mode
...
- *mke2fs* - a very detailed and sophisticated command
  ...
  - e.g. `mke2fs -j -L "vbird_logical" -b 2048 -i 8192 /dev/hdc6`
...
**Unfrequently used commands**
*mknod, e2label, tune2fs, hdparm*
P282 Book for details
